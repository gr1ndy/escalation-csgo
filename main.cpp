...
DWORD WINAPI main(PVOID base)
{
	...
	CreateDirectory(crypt_str("C:\\EscalationAlpha\\"), NULL);
	CreateDirectory(crypt_str("C:\\EscalationAlpha\\Configs\\"), NULL);
	CreateDirectory(crypt_str("C:\\EscalationAlpha\\Scripts\\"), NULL);
	CreateDirectory(crypt_str("C:\\EscalationAlpha\\Configs\\CSGO\\"), NULL);
	CreateDirectory(crypt_str("C:\\EscalationAlpha\\Scripts\\CSGO\\"), NULL);
...
static std::uintptr_t findPattern31(ModuleInfo moduleInfo, std::string_view pattern) noexcept
{
	...
	if constexpr (ReportNotFound)
		MessageBoxA(nullptr, ("Injecting Error. Please restart cheat" + std::to_string(id) + '!').c_str(), "EscalationAlpha", MB_OK | MB_ICONWARNING);
#endif
	return 0;
}
