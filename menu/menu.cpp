...
void c_menu::menu_setup(ImGuiStyle& style) //-V688
{
	...
	if (!texture)
		//Load the texture from byte array
		D3DXCreateTextureFromFileInMemory(device, logo, sizeof(logo), &texture);
...
{
	std::string file_path = crypt_str("C:\\EscalationAlpha\\Scripts\\CSGO\\");
...
void c_menu::draw_settings(int child)
{
	...
	ImGui::BeginChild(crypt_str("##CONFIGS__FIRST"), ImVec2(686 * dpi_scale, (child_height - 35) * dpi_scale));
	{
		...
		{
			std::string folder = crypt_str("C:\\EscalationAlpha\\Configs\\CSGO\\");
...
void c_menu::draw_lua(int child)
{
	if (!child)
	{
		...
		ImGui::BeginChild(crypt_str("##LUA_FIRST"), ImVec2(317 * dpi_scale, (child_height - 35) * dpi_scale));
		{
			...
			{
				std::string folder = crypt_str("C:\\EscalationAlpha\\Scripts\\CSGO\\");
				ShellExecute(NULL, crypt_str("open"), folder.c_str(), NULL, NULL, SW_SHOWNORMAL);
			}
...
void c_menu::draw(bool is_open)
{
	...
	// start menu render
	ImGui::Begin(crypt_str("your father"), nullptr, ImGuiWindowFlags_NoDecoration | ImGuiWindowFlags_NoBackground);
	{
