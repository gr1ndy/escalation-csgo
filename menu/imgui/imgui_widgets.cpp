...
bool ImGui::TabButton(const char* label, bool active)
{
    ImGuiWindow* window = GetCurrentWindow();
    if (window->SkipItems)
        return false;
    ...
    ImVec2 pos = window->DC.CursorPos;
    ImVec2 size = CalcItemSize(ImVec2(154, 32), label_size.x + style.FramePadding.x * 2.0f, label_size.y + style.FramePadding.y * 2.0f);
    ...
    auto this_hover = hover_anim.find(id);
    if (this_hover == hover_anim.end())
    {
        hover_anim.insert({ id, 0.f });
        this_hover = hover_anim.find(id);
    }
    this_hover->second = ImClamp(this_hover->second + g.IO.DeltaTime * 2.f * (pressed || hovered ? 1.f : -1.f), 0.f, 0.35f);
    ...
    this_act->second = ImClamp(this_act->second + g.IO.DeltaTime * 2.f * (active ? 1.f : -1.f), this_hover->second, 1.0f);

    // Render
    const ImU32 col = GetColorU32((held && hovered) ? ImGuiCol_ButtonActive : hovered ? ImGuiCol_ButtonHovered : ImGuiCol_Button);
    RenderNavHighlight(bb, id);
    if (this_act->second > this_hover->second) {
        GetForegroundDrawList()->AddRectFilled(bb.Min, ImVec2(bb.Min.x + 1, bb.Max.y), ImColor(80, 136, 199, int(255 * g.Style.Alpha * (this_act->second))));
    }
    //PushFont(io.Fonts[0].Fonts[1]);
    GetForegroundDrawList()->AddText(ImVec2(bb.Min.x + 13, bb.GetCenter().y - CalcTextSize(label).y / 2), ImColor(76, 76, 76, int(255 * g.Style.Alpha)), label);
    GetForegroundDrawList()->AddText(ImVec2(bb.Min.x + 13, bb.GetCenter().y - CalcTextSize(label).y / 2), ImColor(80, 136, 199, int(255 * g.Style.Alpha * (this_act->second))), label);
    // PopFont();
...
bool ImGui::Checkbox(const char* label, bool* v)
{
    ...
    // active alpha
    if (*v)
        it_anim->second.active_alpha += 20 * (1.f - ImGui::GetIO().DeltaTime);
    else
        it_anim->second.active_alpha -= 20 * (1.f - ImGui::GetIO().DeltaTime);

    if (it_anim->second.active_alpha > 255)
        it_anim->second.active_alpha = 255;
    else if (it_anim->second.active_alpha < 0)
        it_anim->second.active_alpha = 0;

    // hovered alpha
    if (hovered)
        it_anim->second.hovered_alpha += 5 * (1.f - ImGui::GetIO().DeltaTime);
    else
        it_anim->second.hovered_alpha -= 5 * (1.f - ImGui::GetIO().DeltaTime);

    if (it_anim->second.hovered_alpha > 50)
        it_anim->second.hovered_alpha = 50;
    else if (it_anim->second.hovered_alpha < 0)
        it_anim->second.hovered_alpha = 0;

    // rect
    window->DrawList->AddRectFilled(rect.Min, rect.Min + ImVec2(20, 20), ImColor(80, 136, 199, it_anim->second.active_alpha), 4.f);

    // outline
    window->DrawList->AddRect(rect.Min, rect.Min + ImVec2(20, 20), ImColor(50, 50, 50, 255 - it_anim->second.active_alpha), 4.f);

    // checkmark
    RenderCheckMark(window->DrawList, ImVec2(rect.Min + ImVec2(6, 6)), ImColor(0, 0, 0, it_anim->second.active_alpha), 8.f);

    // text
    window->DrawList->AddText(rect.Min + ImVec2(30, 3), ImColor(255, 255, 255, 45 + it_anim->second.active_alpha), label);
...
bool ImGui::InputTextEx(const char* label, const char* hint, char* buf, int buf_size, const ImVec2& size_arg, ImGuiInputTextFlags flags, ImGuiInputTextCallback callback, void* callback_user_data)
{
    ...
    if (g.ActiveId == id)
    {
        ...
        globals.g.focused_on_input = true;
