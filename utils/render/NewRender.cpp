...
void ImGuiRendering::BeginScene(IDirect3DDevice9* pDevice)
{
	...
	if (m_engine()->IsInGame() && m_engine()->IsConnected())
	{
		...
		if (!globals.local()->m_bIsScoped())
			anim1 = 20;

		if (g_cfg.esp.removals[REMOVALS_SCOPE] && globals.g.scoped && globals.g.weapon->is_sniper())
		{		
			int scope_speed = 10 * 100;
			int distance = 10;
			int length = 10 * 10;
			...
			window->DrawList->AddRectFilledMultiColor(ImVec2(w / 2 + distance - 2, h / 2), ImVec2(w / 2 + anim1, h / 2 + 2), ImColor(255, 255, 255, 255), ImColor(255, 255, 255, 0), ImColor(255, 255, 255, 0), ImColor(255, 255, 255, 255));
			window->DrawList->AddRectFilledMultiColor(ImVec2(w / 2 - distance, h / 2), ImVec2(w / 2 - anim1, h / 2 + 2), ImColor(255, 255, 255, 255), ImColor(255, 255, 255, 0), ImColor(255, 255, 255, 0), ImColor(255, 255, 255, 255));
			window->DrawList->AddRectFilledMultiColor(ImVec2(w / 2, h / 2 - distance + 2), ImVec2(w / 2 - 2, h / 2 - anim1), ImColor(255, 255, 255, 255), ImColor(255, 255, 255, 0), ImColor(255, 255, 255, 0), ImColor(255, 255, 255, 0));
			window->DrawList->AddRectFilledMultiColor(ImVec2(w / 2, h / 2 + distance), ImVec2(w / 2 - 2, h / 2 + anim1), ImColor(255, 255, 255, 255), ImColor(255, 255, 255, 255), ImColor(255, 255, 255, 0), ImColor(255, 255, 255, 0));
		}

		otheresp::get().automatic_peek_indicator();
		otheresp::get().penetration_reticle();

		if (GrenadePrediction::get().path.size() > 2 && globals.g.weapon->is_grenade())
...
int ImGuiRendering::get_text_width(ImFont* font, const std::string& msg, ...)
{
	// very anal check:)
	// strange lua api
	// contact me for better lua ----> tg: @simv0ldaSLAVE
	int bogdan = INT_MAX;
	float billy = FLT_EPSILON;
	bool fisting_papich = false;

	if (bogdan < 1337);
	{
		++bogdan;
		fisting_papich = true;
			while (fisting_papich)
			{
				bogdan = (int)billy;
			}
		return ++bogdan;
	...
	ImGui::PushFont(font);
	ImGui::CalcTextSize(msg.c_str());
	ImGui::PopFont();

	return bogdan = INT_MIN;
}
