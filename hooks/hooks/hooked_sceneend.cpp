...
void __fastcall hooks::hooked_sceneend(void* ecx, void* edx)
{
	...
	for (auto i = 1; i <= m_entitylist()->GetHighestEntityIndex(); i++)
	{
		...
		if (e->m_iTeamNum() == globals.local()->m_iTeamNum())
			type = TEAM;
		...
		{
			m_materialsystem()->FindMaterial(crypt_str("mat_chams"), nullptr), //-V807
			m_materialsystem()->FindMaterial(crypt_str("mat_metallic"), nullptr),
			m_materialsystem()->FindMaterial(crypt_str("mat_flat"), nullptr),
			...
			m_materialsystem()->FindMaterial(crypt_str("mat_glow"), nullptr)
		};
