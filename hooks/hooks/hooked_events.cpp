...
void C_HookedEvents::FireGameEvent(IGameEvent* event)
{
	...
	if (!strcmp(event->GetName(), crypt_str("player_footstep")))
	{
		...
		if (e->valid(false, false))
		{
			...
			if (g_cfg.player.type[type].footsteps)
			{
				...
				if (globals.g.should_update_beam_index)
	...
	else if (!strcmp(event_name, crypt_str("weapon_fire")))
	{
		...
		if (user == m_engine()->GetLocalPlayer())
		{
			...
			if (current_shot)
			{
				...
				++globals.g.fired_shots[aim::get().last_target[current_shot->last_target].record.i];
			}
		}
	}
	...
	else if (!strcmp(event_name, crypt_str("player_death")))
	{
		...
		if (globals.local()->is_alive() && attacker_id == m_engine()->GetLocalPlayer() && user_id != m_engine()->GetLocalPlayer())
		{
			auto entity = static_cast<player_t*>(m_entitylist()->GetClientEntity(user_id));

			if (g_cfg.player.enable && g_cfg.esp.kill_effect)
				globals.local()->m_flHealthShotBoostExpirationTime() = m_globals()->m_curtime + g_cfg.esp.kill_effect_duration;

			if (g_cfg.esp.killsound)
			{
				auto headshot = event->GetBool(crypt_str("headshot"));

				switch (globals.g.kills)
				{
				case 0:
	...
	else if (!strcmp(event_name, crypt_str("round_start")))
	{
		...
		for (int i = 1; i < m_globals()->m_maxclients; i++) //red
		{
		...
		globals.g.bomb_timer_enable = false;
...
void C_HookedEvents::RegisterSelf()
{
	...
	globals.g.events.emplace_back(crypt_str("player_footstep"));
	globals.g.events.emplace_back(crypt_str("player_hurt"));
	globals.g.events.emplace_back(crypt_str("player_death"));
	globals.g.events.emplace_back(crypt_str("weapon_fire"));
	globals.g.events.emplace_back(crypt_str("item_purchase"));
	globals.g.events.emplace_back(crypt_str("bullet_impact"));
	globals.g.events.emplace_back(crypt_str("round_start"));
	globals.g.events.emplace_back(crypt_str("round_freeze_end"));
	globals.g.events.emplace_back(crypt_str("bomb_defused"));
	globals.g.events.emplace_back(crypt_str("bomb_begindefuse"));
	globals.g.events.emplace_back(crypt_str("bomb_beginplant"));
}
