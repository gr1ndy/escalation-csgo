...
void bullettracers::draw_beams()
{
	...
	while (!impacts.empty())
	{
		...
		if (TIME_TO_TICKS(m_globals()->m_curtime) > TIME_TO_TICKS(impacts.begin()->time))
		{
			...
			draw_beam(impacts.begin()->e == globals.local(), impacts.begin()->e == globals.local() ? aim::get().last_shoot_position : impacts.begin()->e->get_shoot_position(), impacts.begin()->impact_position, color);
			impacts.erase(impacts.begin());
			continue;
		}
