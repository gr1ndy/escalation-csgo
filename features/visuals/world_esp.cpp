...
void worldesp::molotov_timer(entity_t* entity)
{
	...
	auto col = g_cfg.esp.molotov_timer_color;
	...
	g_Render->DrawString(screen_origin.x, screen_origin.y - size.y * 0.5f - 12, Color(col.r(), col.g(), col.b()), render2::centered_x | render2::centered_y | render2::outline, c_menu::get().Timers, "M");
...
void worldesp::bomb_timer(entity_t* entity)
{
	...
	auto distance = globals.local()->get_shoot_position().DistTo(entity->GetAbsOrigin());
	auto a = 450.7f;
	auto b = 75.68f;
	auto c = 789.2f;
	auto d = ((distance - b) / c);
	...
	float damage;
	damage = float((std::max)((int)ceilf(util::csgo_armor(fl_damage, globals.local()->m_ArmorValue())), 0));
	...
	if (globals.local()->is_alive() && damage >= globals.local()->m_iHealth())
		damage_text = "FATAL";
	...
	auto c4_defuse_time = bomb->m_flDefuseCountDown() - (m_globals()->m_intervalpertick * globals.local()->m_nTickBase());
	...
	if (util::get_bbox(weapon, box, false))
	{
		...
		if (g_cfg.esp.weapon[WEAPON_BOX])
		{
			...
			if (g_cfg.esp.weapon[WEAPON_AMMO] && entity->GetClientClass()->m_ClassID != CBaseCSGrenadeProjectile && entity->GetClientClass()->m_ClassID != CSmokeGrenadeProjectile && entity->GetClientClass()->m_ClassID != CSensorGrenadeProjectile && entity->GetClientClass()->m_ClassID != CMolotovProjectile && entity->GetClientClass()->m_ClassID != CDecoyProjectile)
			{
				...
				g_Render->FilledRect(box.x, box.y + box.h + 6, weapon->m_iClip1() * box.w / weapon->get_csweapon_info()->iMaxClip1, 2, g_cfg.esp.weapon_ammo_color);
			}
		...
		else
		{
			...
			if (g_cfg.esp.weapon[WEAPON_AMMO] && entity->GetClientClass()->m_ClassID != CBaseCSGrenadeProjectile && entity->GetClientClass()->m_ClassID != CSmokeGrenadeProjectile && entity->GetClientClass()->m_ClassID != CSensorGrenadeProjectile && entity->GetClientClass()->m_ClassID != CMolotovProjectile && entity->GetClientClass()->m_ClassID != CDecoyProjectile)
			{
				...
				g_Render->FilledRect(box.x, box.y + box.h / 2 + pos, weapon->m_iClip1() * box.w / weapon->get_csweapon_info()->iMaxClip1, 2, g_cfg.esp.weapon_ammo_color);
			}
