...
void zeusbot::run(CUserCmd* cmd)
{
	...
	if (globals.g.weapon->m_iItemDefinitionIndex() != WEAPON_TASER)
		return;
...
void zeusbot::scan(adjust_data* record, scan_data& data)
{
	...
	for (hitbox = HITBOX_PELVIS; hitbox <= HITBOX_UPPER_CHEST; ++hitbox)
	{
		...
		if (!record->bot)
		{
			...
			if (record->matrixes_data.zero[0].GetOrigin() == record->matrixes_data.first[0].GetOrigin() || record->matrixes_data.zero[0].GetOrigin() == record->matrixes_data.second[0].GetOrigin() || record->matrixes_data.first[0].GetOrigin() == record->matrixes_data.second[0].GetOrigin())
				safe = 0.0f;
			else if (!hitbox_intersection(record->player, record->matrixes_data.zero, hitbox, globals.g.eye_pos, point.point, &safe))
				safe = 0.0f;
			else if (!hitbox_intersection(record->player, record->matrixes_data.first, hitbox, globals.g.eye_pos, point.point, &safe))
				safe = 0.0f;
			else if (!hitbox_intersection(record->player, record->matrixes_data.second, hitbox, globals.g.eye_pos, point.point, &safe))
				safe = 0.0f;
	...
	for (auto& point : points)
	{
		...
		auto fire_data = autowall::get().wall_penetration(globals.g.eye_pos, point.point, record->player);
