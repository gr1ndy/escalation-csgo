...
public:
	...
	scanned_target(adjust_data* record, const scan_data& data) //-V688 //-V818
	{
		...
		auto aim_angle = math::calculate_angle(globals.g.eye_pos, data.point.point); //-V688
		auto fov = math::get_fov(viewangles, aim_angle); //-V688

		this->fov = fov;
		this->distance = globals.g.eye_pos.DistTo(data.point.point);
...
public:
	...
	void scan(adjust_data* record, scan_data& data, const Vector& shoot_position = globals.g.eye_pos, bool optimized = false);
