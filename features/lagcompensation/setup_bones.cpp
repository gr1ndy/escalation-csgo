...
void CSetupBones::accumulate_layers(player_t* player, uintptr_t edx, CSetupBones** setup, Vector* pos, float time)
{
	// fix accumulate_layers
	if (!player || !player->is_player() ||
		!player->is_alive() || player->IsDormant())
		return;
	...
	for (auto i = 0; i < 13; i++)
	{
		const auto& layer = (*player->get_animlayers());

		if (layer.m_flWeight > 0 && layer.m_nOrder >= 0 && layer.m_nOrder < layers.size())
		...
		if (weaponWorldModel)
		{
			...
			if (bone_merge)
			{
				...
				for (auto i = 0; i < m_nAnimOverlayCount; ++i)
				{
					...
					{
						if (m_bShouldDispatch && m_animating == globals.local())
						{
